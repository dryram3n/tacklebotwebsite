document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateWaves(elapsed) {
        if (!wavePath1 || !wavePath2) return; // Check elements exist
  
        // Calculate time factor for smooth transitions regardless of frame rate
        const timeFactor = Math.min(2, elapsed / FRAME_DURATION); // Clamp timeFactor to prevent large jumps
    ...
    function initWaterBackground() {
        ...
        updateWaterHeight = (elapsed) => {
            ...
            const timeFactor = Math.min(2, elapsed / FRAME_DURATION); // Normalize easing, clamp max factor
    ...
    function updateAllFish(elapsed) {
        ...
        const elapsedSeconds = elapsed / 1000; // Convert ms to seconds for speed calculations
    ...
    const DROPLET_INTERVAL = 60; // ms between droplets max
    ...
    function initMouseTrail() {
        ...
        // Function to animate the trail (called in main loop)
        animateTrailFn = (elapsed) => {
            if (!trail || isNaN(mouseX) || isNaN(mouseY)) return; // Safety checks
  
            const timeFactor = Math.min(2, elapsed / FRAME_DURATION); // Clamp easing factor
            ...
            trail.style.transform = `translate(${trailX.toFixed(0)}px, ${trailY.toFixed(0)}px) translate(-50%, -50%) rotate(45deg) scale(${trail.style.transform.includes('scale(0.6)') ? 0.6 : 1})`;
            ...
            if (now - lastDropletTime > DROPLET_INTERVAL && distMoved > 5 && Math.random() > 0.6) { // Only create if moved and random chance
         ...
         document.addEventListener('mousemove', () => {
             ...
             fadeTimeout = setTimeout(() => { if (trail) trail.style.opacity = '0'; }, 300);
         });
  
         // Click/Splash logic
         document.addEventListener('mousedown', (e) => {
             if (!trail || e.target.closest('a, button, .button, input, select, textarea, [role="button"]')) return; // Ignore clicks on interactive elements
         ...
         document.addEventListener('mouseup', (e) => {
             if (!trail || e.target.closest('a, button, .button, input, select, textarea, [role="button"]')) return;
             ...
             setTimeout(() => {
                 if (trail) trail.style.transform = `translate(${trailX.toFixed(0)}px, ${trailY.toFixed(0)}px) translate(-50%, -50%) rotate(45deg) scale(1)`;
             }, 150);
         });
         ...
         animateTrailFn(0); // Run once to position trail initially
    }
    ...
    function initStaticListeners() {
        ...
        document.querySelectorAll('.expand-btn').forEach(button => {
            ...
            const isVisible = expandedContent?.style.display === 'block';
            button.setAttribute('aria-expanded', isVisible);
            // Ensure initial text matches state
             button.textContent = isVisible
                ? button.textContent.replace('View', 'Hide').replace('+', '−')
    ...
    function mainLoop(timestamp) {
        ...
        // Only perform updates if enough time has passed since the last update cycle
        if (elapsed >= FRAME_DURATION * 0.9) { // Allow slightly less than target duration
            lastTimestamp = timestamp; // Update last timestamp for the next cycle
  
            // --- Perform Updates ---
            // These functions are called on every throttled frame
            ...
            // Check if animateTrailFn is initialized before calling
            if (typeof animateTrailFn === 'function') {
                animateTrailFn(elapsed);
            }
