...
class WaterCanvas {
    ...
    update(timestamp) {
      ...
      const elapsed = timestamp - this.lastTimestamp; // Time since last frame in ms
      this.lastTimestamp = timestamp;
  
      // Clamp elapsed time to prevent large jumps if tab was inactive
      const delta = Math.min(elapsed, 50); // Max 50ms step (equiv to 20fps min)
